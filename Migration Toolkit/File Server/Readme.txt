...
1) - run Get-GroupSIDHistory.ps1 and Get-UserSIDHistory on both domains to get two different SID outputs from each domain (no modules needed - only powershell)

2) - on the server your running this from import the SIDHistory Module by doing cd .\SIDHistory then run import-module .\SIDHistory.psm1


3) - the Get-GroupSIDHistory.ps1 and Get-UserSIDHistory scripts output the SIDs in a csv to C:\SIDHistoryGroups and C:\SIDHistoryUsers - copy the outputs from Domain1 into the same folder in domain2 so two exist in the same folder like this:
...
7) - you can then run the below commands to convert all the SIDS found to exist in the previous domain to the users in the new domain on a file server
...
in the scenario i worked on users had multiple SIDS as the fileserver had changed hands a few times and the data received was very inconsistent - i had to manually add in the mappings for the users and groups with extra SIDS to the sidmap file which i'd got from an additional NTFS permissions output script - for example the SIDmap would read:

userSID1olddomain,userSID1newdomain
userSID2olddomain,userSID1newdomain
userSID3olddomain,userSID1newdomain
